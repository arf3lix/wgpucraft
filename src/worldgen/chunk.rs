...
impl ChunkArray {
    ...
    pub fn get_block(&self, world_pos: Vector3<i32>) -> Option<MaterialType> {
        ...
        self.get_chunk_index_by_offset(&chunk_offset)
            .and_then(|index| {
                ...
                Some(blocks[y as usize][x as usize][z as usize].material_type)
            })
    }
    ...
    pub fn set_block(&mut self, world_pos: Vector3<i32>, material: MaterialType) -> Option<usize> {
        ...
        if let Some(index) = self.get_chunk_index_by_offset(&chunk_offset) {
            ...
            let block = &mut blocks[y as usize][x as usize][z as usize];
...
pub fn generate_blocks_independent(offset: [i32; 3], noise_generator: &NoiseGenerator, biome: &BiomeParameters) -> Blocks {
    let mut blocks = Blocks::new();
    generate_chunk(&mut blocks, offset, noise_generator, biome);
    blocks
}